...
// implements simple Lambert shading (NOTE: Assumes a single directional light source defined in EC (liHat))
vec4 evaluateLightingModel()
{
	vec3 vHat;
	if (ec_lds[3][3] == 0) // if current projection is perspective
	{
		vHat = normalize(-pvaIn.ecPosition);
	}
	else // current projection is orthogonal or oblique
	{
	...
	// decide if we need to "correct" the normal vector n-hat
